...
from model.basic_cnn import cnn_asl
...
with tf.Session(config = session_config) as sess:
    train_x, train_y = create_inputs_asl(is_train)
    ...
    prediction = cnn_asl(x, keep_prob=0.8)
    # 求loss
    loss = cfg.cross_entropy_loss(prediction, y)
    # 设置优化器
    global_step, train_step = cfg.set_optimizer(num_batches_per_epoch=n_batch_train, loss=loss)
    # 求acc
    accuracy = cfg.get_acc(prediction, y)
    # 初始化变量
    coord, threads = cfg.init_variables_and_start_thread(sess)
    # 恢复model
    saver = cfg.restore_model(sess,ckpt,restore_model =restore_model)
    # 设置训练日志
    ...
    for epoch_n in range(start_epoch,epoch):
        for n_batch in range(n_batch_train):
            ...
            # 显示结果batch_size
            cfg.print_message(epoch_n,step,loss_value,acc_value)
        # 保存summary
        if (step + 1) % 20 == 0:
