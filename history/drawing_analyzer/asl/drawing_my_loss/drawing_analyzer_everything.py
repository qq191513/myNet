...
def get_parameters_ASLt(path,re_dict={}):
	...
	for key in re_dict.keys():
		with open(path, 'r') as fo:
			...
			for line in fo:
				...
				if res:
					number_ASLt.append(float(res[0]))
		data_dict[key] =number_ASLt
	return data_dict

def curve_smooth(data_ASLt, batch_size=100):
	new_data_ASLt, idx_ASLt = [], []
	for i in range(int(len(data_ASLt) / batch_size)):
		batch = data_ASLt[i*batch_size: (i+1)*batch_size]
		new_data_ASLt.append(1.0 * sum(batch) / len(batch))
		idx_ASLt.append(i*batch_size)

	return new_data_ASLt, idx_ASLt
...
if __name__ =='__main__':
    file_path = 'run_.-tag-all_loss_em_ori.txt'
    file_path_1 = 'capsnet_em_v3-all_loss.txt'
    ...
    y_datas_dict_1 = get_parameters_ASLt(path = file_path ,re_dict= y_re_dict_1)

    #file_path_2
    y_re_dict_2 = OrderedDict()
    y_re_dict_2['capsnet_v1_Value']=r'\'Value\': ([\d\.]+)'
    y_datas_dict_2 = get_parameters_ASLt(path = file_path_1 ,re_dict= y_re_dict_2)

    #file_path_3
    # y_re_dict_3 = OrderedDict()
    # y_re_dict_3['capsnet_v3_Value']=r'\'Value\': ([\d\.]+)'
    # y_datas_dict_3 = get_parameters_ASLt(path = file_path_2 ,re_dict= y_re_dict_3)
    ...
    x_datas_dict = get_parameters_ASLt(path = file_path ,re_dict= x_re_dict)
    ...
    setting_dict['title'] = 'The loss value of the Caps-em in ASL dataset'
