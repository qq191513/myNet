...
def shuffle_bottleneck(inputs, depth_bottleneck, group, stride, shuffle=True, outputs_collections = None, scope= None):
        ...
        with tf.variable_scope(scope, 'Unit', [inputs]) as sc:
            ...
            net = group_pointwise_conv2d(inputs, depth_bottleneck, 1, group = (1 if (operator.eq(sc.name ,'ShuffleNet/Stage2/Unit0')== 0) else group), relu=True, scope="1x1GConvIn")
...
shufflenet.default_image_size = 32
...
if __name__ == "__main__":
    ...
    logits, end_points= shufflenet_g2(inputs)
