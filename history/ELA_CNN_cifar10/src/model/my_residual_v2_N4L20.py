...
class ConvNet():
    def __init__(self, n_channel=3, n_classes=10, image_size=24, n_layers=20):
        ...
        self.objective = self.objective_1 + self.objective_2 +\
                         self.objective_3+ self.objective_4
    ...
    def test(self, dataloader, backup_path, epoch, batch_size=128):
        ...
        for i in range(0, dataloader.n_test, batch_size):
            ...
            [avg_accuracy_1,avg_accuracy_2,avg_accuracy_3,avg_accuracy_4,logits_1,logits_2,logits_3,logits_4] = self.sess.run(
                fetches=[self.accuracy_1,self.accuracy_2,self.accuracy_3,self.accuracy_4,
                         self.logits_1,self.logits_2,self.logits_3,self.logits_4],
                feed_dict={self.images:batch_images,
                           self.labels:batch_labels,
                           self.keep_prob:1.0})
            ...
            #几列预测值拼接成矩阵
            merrge_array = np.concatenate([[predict_1],[predict_2],[predict_3],[predict_4]],axis=0)
