...
class ConvNet():
    def __init__(self, n_channel=3, n_classes=10, image_size=24, n_layers=44,is_training =True):
        ...
        #子层1--孙2 densenet_bc
        self.logits_1_2 = self.grand_son_2(hidden_conv_1,scope_name='grand_son_2')
        ...
        self.objective = \
            self.objective_1 + self.objective_1_1 +self.objective_1_2 \
            +self.objective_2 \
            +self.objective_3
    ...
    def grand_son_2(self, hidden_conv_1_2, scope_name):
        with tf.variable_scope(scope_name):
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                                activation_fn=None,
                                normalizer_fn=None,
    ...
    def get_5_acc_list(self,test_images,test_labels,n_test,batch_size,is_train =True):
        ...
        for i in range(0, n_test, batch_size):
            ...
            [labels_array,
             avg_accuracy_1, avg_accuracy_1_1,avg_accuracy_1_2,
             avg_accuracy_2,
             avg_accuracy_3,
             logits_1, logits_1_1,logits_1_2,
             logits_2,
             logits_3] = self.sess.run(
                fetches=[self.labels,
                         self.accuracy_1, self.accuracy_1_1,self.accuracy_1_2,
                         self.accuracy_2,
                         self.accuracy_3,
                         self.logits_1,self.logits_1_1,self.logits_1_2,
                         self.logits_2,
                         self.logits_3],
            ...
            # 几列预测值拼接成矩阵
            merrge_array = np.concatenate([[predict_1],[predict_1_1],[predict_1_2],
                                           [predict_2], [predict_3]
                                           ], axis=0)
        ...
        return accuracy_1_list,accuracy_1_1_list,accuracy_1_2_list, \
               accuracy_2_list, \
               accuracy_3_list, \
         acc_decision_batch_dict
    ...
    def train(self, dataloader, backup_path, n_epoch=5,
        ...
        for epoch in range(start_n_epoch, n_epoch+1):
            ...
            # 获取验证准确率列表
            if epoch % 5 == 0:
                accuracy_1_list,accuracy_1_1_list,accuracy_1_2_list, \
                ...
                message_1_2 = ' net1_2: %.4f\n' % (np.mean(accuracy_1_2_list))
                ...
                print_and_save_txt(str=message_epoch + message_1 + message_1_1 + message_1_2
                                       +message_2+
                                       message_3+
    ...
    def test(self, dataloader, backup_path, epoch, batch_size=128,is_training= False):
        ...
        message_hv5 = ' acc_decision_batch_hv5: %.4f\n' % (
            np.mean(acc_decision_batch_dict['acc_decision_batch_hv5']))
        print_and_save_txt(str=  message_1 + message_1_1 + message_1_2
                               + message_2 +
                               message_3 +
                               message_hv2 + message_hv3 + message_hv4 + message_hv5,
