...
class ConvNet():
    def __init__(self, n_channel=3, n_classes=10, image_size=24, n_layers=44,is_training =True,setting=None):
        ...
        input_dense_3_3 = self.grand_son_3_3(hidden_conv_3, scope_name='grand_son_3_3')
        ...
        lr = tf.cond(tf.less(self.global_step, 50000),
                     ...
                     lambda: tf.cond(tf.less(self.global_step, 70000),
        ...
        with tf.variable_scope(scope_name) as sc:
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                ...
                with slim.arg_scope([slim.conv2d, slim.avg_pool2d, slim.max_pool2d, self._fire_module]):
                    with slim.arg_scope([slim.dropout], is_training=self.is_training):
                        # fire5
    ...
    def grand_son_3_1(self, hidden_conv_3_1, scope_name):
        with tf.variable_scope(scope_name) as sc:
            with slim.arg_scope(self.shufflenet_arg_scope(is_training=True)):
                net =self.shuffle_stage(hidden_conv_3_1, depth=200, groups=4, repeat=3, shuffle=True, scope=None)
    ...
    def grand_son_3_3(self, hidden_conv_3_3, scope_name):
        ...
        #             hidden_conv_3_3 = self.block_inception_b(hidden_conv_3_3, block_scope)
        #         hidden_conv_3_3 = self.transition_layer(hidden_conv_3_3, 200, is_training)
        #
        #         ##############################
        #     # with tf.variable_scope('Logits'):
        #
        #         # Global average pooling.
        #         # hidden_conv_3_3 = tf.reduce_mean(hidden_conv_3_3, [1, 2], keep_dims=True, name='global_pool')
        #
        #         # 1 x 1 x 512
        #         # if self.is_training:
        #         #     hidden_conv_3_3 = slim.dropout(hidden_conv_3_3, keep_prob=0.9, scope='Dropout_1b')
        #         # logits = slim.conv2d(net, self.n_classes, [1, 1], activation_fn=None,
        #         #                      normalizer_fn=None, scope='Conv2d_1c_1x1')
        #     with tf.variable_scope("trainsition_layer_to_classes"):
        #         logits = self.trainsition_layer_to_classes(hidden_conv_3_3, self.n_classes,
        #                                                    is_training)
        #         logits = tf.squeeze(logits, [1, 2], name='SpatialSqueeze')
        #
        #     return logits
        ...
        with tf.variable_scope(scope_name):
            with slim.arg_scope(
                    ...
                    padding='SAME'):
                with tf.variable_scope('Branch_0'):
                    branch_0 = slim.conv2d(hidden_conv_3_3, depth(176), [1, 1], scope='Conv2d_0a_1x1')
                with tf.variable_scope('Branch_1'):
                    branch_1 = slim.conv2d(
                        hidden_conv_3_3, depth(96), [1, 1],
                        weights_initializer=trunc_normal(0.09),
                        scope='Conv2d_0a_1x1')
                    branch_1 = slim.conv2d(branch_1, depth(160), [3, 3],
                                           scope='Conv2d_0b_3x3')
                with tf.variable_scope('Branch_2'):
                    ...
                    branch_2 = slim.conv2d(branch_2, depth(112), [3, 3],
                                           scope='Conv2d_0c_3x3')
                with tf.variable_scope('Branch_3'):
                    branch_3 = slim.avg_pool2d(hidden_conv_3_3, [2, 2], scope='AvgPool_0a_3x3')
                    branch_3 = slim.conv2d(
                        branch_3, depth(64), [1, 1],
                        weights_initializer=trunc_normal(0.1),
                        scope='Conv2d_0b_1x1')
                hidden_conv_3_3 = tf.concat(
                    axis=concat_dim, values=[branch_0, branch_1, branch_2, branch_3])

            input_dense_3_3= tf.reduce_mean(hidden_conv_3_3, reduction_indices=[1, 2])
            # self.logits_2 = dense_layer_2.get_output(input=input_dense_3_3)
            return input_dense_3_3
    ...
    def get_all_acc_list(self,test_images,test_labels,n_test,batch_size,is_train =True):
        ...
        for i in range(start_pos, n_test, batch_size):
            ...
            [labels_array,
             ...
             logits_3, logits_3_1,logits_3_2,logits_3_3,
                ...
                fetches=[self.labels,
                         ...
                         self.accuracy_3, self.accuracy_3_1,self.accuracy_3_2,self.accuracy_3_3,
                         self.logits_1,self.logits_1_1,self.logits_1_2,self.logits_1_3,
                         self.logits_2,self.logits_2_1,self.logits_2_2,self.logits_2_3,
                         self.logits_3,self.logits_3_1,self.logits_3_2,self.logits_3_3,
                         ],
            ...
            accuracy_3_1_list.append(avg_accuracy_3_1)
            accuracy_3_2_list.append(avg_accuracy_3_2)
            accuracy_3_3_list.append(avg_accuracy_3_3)
            ...
            predict_3_1 = self.sess.run(tf.argmax(logits_3_1, axis=1))
            predict_3_2 = self.sess.run(tf.argmax(logits_3_2, axis=1))
            predict_3_3 = self.sess.run(tf.argmax(logits_3_3, axis=1))
            # 几列预测值拼接成矩阵
    ...
    def train(self, dataloader, backup_path, n_epoch=5,
        ...
        for epoch in range(start_n_epoch, n_epoch+1):
            ...
            start_pos=0
            if self.setting.debug_mode:
                start_pos = int(dataloader.n_train * 0.95)
            ...
            if epoch % 5 == 0:
                ...
                message_3 = ' net3: %.4f' % (np.mean(accuracy_3_list))
                message_3_1 = ' net3_1: %.4f' % (np.mean(accuracy_3_1_list))
                message_3_2 = ' net3_2: %.4f' % (np.mean(accuracy_3_2_list))
                message_3_3 = ' net3_3: %.4f\n' % (np.mean(accuracy_3_3_list))
