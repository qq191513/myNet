    ...
    def get_5_acc_list(self,test_images,test_labels,n_test,batch_size,is_train =True):
        ...
        for i in range(0, n_test, batch_size):
            ...
            for row in range(0, rows):
                ...
                if result[max_key] <2 :    #拒绝就要删对应标签，方便后面做对比
                    labels_array_hv2 = np.delete(labels_array_hv2,row-delete_off_v2,axis=0)
                    delete_off_v2 +=1  # 计算当前这一批拒绝预测的次数
                    aborted_num_hv2 +=1      # 计算当前这一批拒绝预测的次数
                else:
                    final_batch_predict_hv2.append(max_key)  # 预测值存到列表里面

                if result[max_key] <3:  #拒绝就要删对应标签，方便后面做对比
                    labels_array_hv3 = np.delete(labels_array_hv3, row - delete_off_v3, axis=0)
                    delete_off_v3 += 1      #删除后的偏移
                    aborted_num_hv3 += 1   # 计算当前这一批拒绝预测的次数
                else:
                    final_batch_predict_hv3.append(max_key)  # 预测值存到列表里面

                if result[max_key] < 4:    #拒绝就要删对应标签，方便后面做对比
                    labels_array_hv4 = np.delete(labels_array_hv4, row - delete_off_v4, axis=0)
                    delete_off_v4 += 1      #删除后的偏移位置
                    aborted_num_hv4 += 1    # 计算当前这一批拒绝预测的次数
                else:
                    final_batch_predict_hv4.append(max_key)  # 预测值存到列表里面

                if result[max_key] < 5:     #拒绝就要删对应标签，方便后面做对比
                    labels_array_hv5 = np.delete(labels_array_hv5, row - delete_off_v5, axis=0)
                    delete_off_v5 += 1      #删除后的偏移位置
                    aborted_num_hv5 += 1   # 计算当前这一批拒绝预测的次数
                else:
                    final_batch_predict_hv5.append(max_key)  # 预测值存到列表里面

                if result[max_key] < 6:     #拒绝就要删对应标签，方便后面做对比
                    labels_array_hv6 = np.delete(labels_array_hv6, row - delete_off_v6, axis=0)
                    delete_off_v6 += 1      #删除后的偏移位置
                    aborted_num_hv6 += 1   # 计算当前这一批拒绝预测的次数
                else:
                    final_batch_predict_hv6.append(max_key)  # 预测值存到列表里面

                if result[max_key] < 7:  # 拒绝就要删对应标签，方便后面做对比
                    labels_array_hv7 = np.delete(labels_array_hv7, row - delete_off_v7, axis=0)
                    delete_off_v7 += 1  # 删除后的偏移位置
            ...
            hv2 = get_acc_decision_batch(
                labels_array_hv2, final_batch_predict_hv2)
            print('hv2 %f aborted_num:%d '% (hv2,aborted_num_hv2))
            aborted_num_total_hv2 += aborted_num_hv2  # 计算拒绝预测的次数总和
            if np.isnan(hv2) == False:
                acc_decision_batch_hv2.append(hv2)

            hv3 = get_acc_decision_batch(
                labels_array_hv3, final_batch_predict_hv3)
            print('hv3 %f aborted_num:%d ' % (hv3,aborted_num_hv3))
            aborted_num_total_hv3 += aborted_num_hv3  # 计算拒绝预测的次数总和
            if np.isnan(hv3) == False:
                acc_decision_batch_hv3.append(hv3)

            hv4 = get_acc_decision_batch(
                labels_array_hv4, final_batch_predict_hv4)
            print('hv4 %f aborted_num:%d ' % (hv4,aborted_num_hv4))
            aborted_num_total_hv4 += aborted_num_hv4 ## 计算拒绝预测的次数总和
            if np.isnan(hv4) == False:
                acc_decision_batch_hv4.append(hv4)

            hv5 = get_acc_decision_batch(
                labels_array_hv5, final_batch_predict_hv5)
            print('hv5 %f aborted_num:%d ' % (hv5, aborted_num_hv5))
            aborted_num_total_hv5 += aborted_num_hv5 # 计算拒绝预测的次数总和
            if np.isnan(hv5) == False:
                acc_decision_batch_hv5.append(hv5)

            hv6 = get_acc_decision_batch(
                labels_array_hv6, final_batch_predict_hv6)
            print('hv6 %f aborted_num:%d ' % (hv6, aborted_num_hv6))
            aborted_num_total_hv6 += aborted_num_hv6  # 计算拒绝预测的次数总和
            if np.isnan(hv6) == False:
                acc_decision_batch_hv6.append(hv6)

            hv7 = get_acc_decision_batch(
                labels_array_hv7, final_batch_predict_hv7)
            print('hv7 %f aborted_num:%d ' % (hv7, aborted_num_hv7))
            aborted_num_total_hv7 += aborted_num_hv7  # 计算拒绝预测的次数总和
