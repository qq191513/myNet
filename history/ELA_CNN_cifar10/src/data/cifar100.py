...
class Corpus:
    ...
    def __init__(self):
        self.load_cifar100('/home/mo/work/data_set/cifar-100-python')
    ...
    def load_cifar100(self,directory):
        # 读取训练集
        images, labels = [], []
        filename ='%s/train' % directory
        with open(filename, 'rb') as fo:
            if 'Windows' in platform.platform():
                cifar100 = pickle.load(fo, encoding='bytes')
            elif 'Linux' in platform.platform():
                cifar100 = pickle.load(fo, encoding='bytes')

            for i in range(len(cifar100[b"labels"])):
                image = numpy.reshape(cifar100[b"data"][i], (3, 32, 32))
                image = numpy.transpose(image, (1, 2, 0))
                image = image.astype(float)
                images.append(image)
            labels += cifar100[b"labels"]
