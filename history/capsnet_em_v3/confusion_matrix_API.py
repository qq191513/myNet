...
def plot_confusion_chinese(cm, title='Confusion Matrix',labels= None,cmap=None,savefig=None,font_dict=None,chinese_ttf=None):
    # 中文显示，从window的C:\Windows\Fonts里面挑选一个喜欢的字体复制到Linux系统里面，设置好下面的路径

    # 创建图
    plt.figure(figsize=(9, 4), dpi=150) #例如figsize=(5, 5)相当于生成一张500*500大小的图片， 调试的时候用低分辨dpi=150，应用的时候再用高分辨率1500
    ...
    myfont = FontProperties(fname=chinese_ttf, size=15)
    # 显示ylabel
    plt.ylabel('标签值',fontdict={'fontsize':font_dict['ylabel']},fontproperties = myfont)
    # 显示xlabel
    plt.xlabel('预测值',fontdict={'fontsize':font_dict['xlabel']},fontproperties = myfont)
