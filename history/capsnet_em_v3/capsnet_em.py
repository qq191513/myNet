...
def spread_loss(output, pose_out, x, y, m):
    ...
    #防止NaN gradients
    # output1 = tf.clip_by_value(output1, 1e-8, 1.0)
    # output1= output1+1e-8
    y = tf.expand_dims(y, axis=2)
    ...
    with tf.variable_scope('decoder'):
        ...
        pose_out = slim.fully_connected(pose_out, data_size * data_size * data_channel,
...
def kernel_tile(input, kernel, stride):
    ...
    # output = tf.clip_by_value(output, 1e-8, 10.0)
...
def build_arch(input, coord_add, is_train: bool, num_classes: int):
    ...
    with slim.arg_scope([slim.conv2d], trainable=is_train, biases_initializer=bias_initializer, weights_regularizer=weights_regularizer):
        with tf.variable_scope('relu_conv1') as scope:
            ...
            # output_relu_conv1 = print_tensor(output_relu_conv1, message='output_relu_conv1')
            # output_relu_conv1 = slim.batch_norm(output_relu_conv1, center=False, is_training=is_train, trainable=False)
        ...
        with tf.variable_scope('primary_caps') as scope:
            ...
            # pose = print_tensor(pose, message='primary_caps pose')
            # pose = tf.clip_by_value(pose, 1e-8, 10.0)
            ...
            # activation = tf.clip_by_value(activation, 1e-8, 10.0)

            # activation = print_tensor(activation, message='primary_caps activation')
            ...
            # Identity_output = print_tensor(Identity_output, message='primary_caps Identity_output')


        with tf.variable_scope('conv_caps1') as scope:
            output = kernel_tile(Identity_output, 3, 2)
            # output = kernel_tile(output_primary_caps, 3, 2)
            # output = print_tensor(output, message='conv_caps1 kernel_tile')
            ...
            # output = slim.batch_norm(output, center=False, is_training=is_train, trainable=False)


        with tf.variable_scope('conv_caps2') as scope:
            output = kernel_tile(output, 3, 1)
            # output = print_tensor(output, message='conv_caps2 kernel_tile')
            ...
            # activation = slim.batch_norm(activation, center=False, is_training=is_train, trainable=False)
        ...
        with tf.variable_scope('class_caps') as scope:
            #其实class_caps这层跟卷囊卷积层的运作是一样的，只是让输出变成numclass而已，再加一个coord_add提升效果
            with tf.variable_scope('v') as scope:
            #1、先姿态变换
                # conv cap 2 pose shape: (288, 144, 16, 16)
                votes = mat_transform(pose, num_classes, weights_regularizer)
                # votes = print_tensor(votes, message='class_caps votes')
                # votes (288, 16, 22, 16)
                ...
                # votes = print_tensor(votes, message='coord_add_op votes')
            ...
            with tf.variable_scope('routing') as scope:
                ...
                # class cap activation shape: (288, 22)
        ...
        #经过全局平局池化(288, 22)由得class cap output shape: (32, 22)
...
def test_accuracy(logits, labels):
    ...
    # logits_idx = print_tensor(logits_idx,message='logits_idx')
    # labels = print_tensor(labels,message='labels    ')
...
def batch_results_and_labels(logits, labels):

    logits_idx = tf.to_int32(tf.argmax(logits, axis=1))
    results = tf.reshape(logits_idx, shape=(cfg.batch_size,))
