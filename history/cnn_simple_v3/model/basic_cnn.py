...
def cnn_L4(x,input_shape,class_num,keep_prob):
    x_image = tf.reshape(x, [input_shape[0], input_shape[1], input_shape[2], input_shape[3]])  # 转换输入数据shape,以便于用于网络中
    # L1
    W_conv1 = weight_variable([3, 3,input_shape[3], 32])
    b_conv1 = bias_variable([32])
    h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1)  # 第一个卷积层
    # L2
    ...
    h_pool2 = max_pool(h_conv2)  # 第一个池化层
    # L3
    ...
    W_conv2 = weight_variable([3, 3, 32, 32])
    b_conv2 = bias_variable([32])
    h_conv2 = tf.nn.relu(conv2d(h_conv3, W_conv2) + b_conv2)  # 第四个卷积层
    h_pool2 = max_pool(h_conv2)  # 第二个池化层

    W_fc1 = weight_variable([7 * 7 * 32, 1024])
    b_fc1 = bias_variable([1024])
    h_pool2_flat = tf.reshape(h_pool2, [-1, 7 * 7 * 32])  # reshape成向量
    h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)  # 第一个全连接层
    h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)  # dropout层
