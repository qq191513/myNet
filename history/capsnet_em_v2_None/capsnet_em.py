"""
当原始网络训练完后，加大reconstruction_proportion比例，和重新调大lr，后期重新训练reconstruction
"""
...
import numpy as np
####################   改这里  ##########################################
# reconstruction_proportion = 0.0005
# reconstruction_proportion = 0.05  #没什么卵用
# reconstruction_proportion = 1  #不行
reconstruction_proportion = 0.5  #
####################   end       ########################################
...
def cross_ent_loss(output, pose_out, x, y, m):
    """
    # check NaN
    ...
    if cfg.weight_reg:
        ...
        loss_all = tf.add_n([loss] + [reconstruction_proportion * data_size* data_size * reconstruction_loss] + regularization)
    else:
        loss_all = tf.add_n([loss] + [reconstruction_proportion * data_size* data_size * reconstruction_loss])
