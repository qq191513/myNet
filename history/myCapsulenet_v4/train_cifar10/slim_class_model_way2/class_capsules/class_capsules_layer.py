...
class capsule_layer(object):
    ...
    def _matmul_broadcast(self,x, y, name):
    ...
    def _get_variable_wrapper(self,
    ...
    def _conv2d_wrapper(self,inputs, shape, strides, padding, add_bias, activation_fn, name):
    ...
    def _separable_conv2d_wrapper(self,inputs, depthwise_shape, pointwise_shape, strides, padding, add_bias, activation_fn,
                                  name):
    ...
    def _depthwise_conv2d_wrapper(self,inputs, shape, strides, padding, add_bias, activation_fn, name):
    ...
    def matrix_capsules_em_routing(self,votes, i_activations, beta_v, beta_a, iterations, name):
      ...
      with tf.variable_scope(name) as scope:
        ...
        def m_step(rr, votes, i_activations, beta_v, beta_a, inverse_temperature):
          ...
          rr_prime_sum = tf.reduce_sum(
            rr_prime, axis=-3, keepdims=True, name='rr_prime_sum'
          )
          ...
          o_cost_h = (beta_v + tf.log(o_stdv + self.epsilon)) * rr_prime_sum
          # o_cost_h = tf.Print(
          #   o_cost_h, [beta_v, o_cost_h.shape, o_cost[0, ..., :, 0, :]], 'mstep: beta_v, o_cost_h', summarize=20
          # )

          # o_activation: [N, OH, OW, 1, O, 1]
          # o_activations_cost = (beta_a - tf.reduce_sum(o_cost_h, axis=-1, keepdims=True))
        ...
        def e_step(o_mean, o_stdv, o_activations, votes):
          ...
          o_p_unit2 = - tf.reduce_sum(
            tf.log(o_stdv + self.epsilon), axis=-1, keepdims=True
          )
          # o_p_unit2 = tf.Print(
          ...
          zz = tf.log(o_activations + self.epsilon) + o_p
          rr = tf.nn.softmax(
