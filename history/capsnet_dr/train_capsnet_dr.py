...
def main(args):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        """Set summary writer"""
        if not os.path.exists(cfg.logdir + '/caps_dr/{}/train_log/'.format(dataset_name)):
            os.makedirs(cfg.logdir + '/caps_dr/{}/train_log/'.format(dataset_name))
        summary_writer = tf.summary.FileWriter(
            cfg.logdir + '/caps_dr/{}/train_log/'.format(dataset_name), graph=sess.graph)  # graph = sess.graph, huge!
            ...
            try:
                ...
                if (step % num_batches_per_epoch) == 0:
                    ...
                    ckpt_path = os.path.join(
                        cfg.logdir + '/caps_dr/{}/'.format(dataset_name), 'model-{:.4f}.ckpt'.format(loss_value))
