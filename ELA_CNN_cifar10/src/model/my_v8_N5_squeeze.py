...
class ConvNet():
    def __init__(self, n_channel=3, n_classes=10, image_size=24, n_layers=20):
        ...
        with slim.arg_scope(squeezenet_arg_scope()):
            self.logits_1, end_points = squeezenet(basic_conv, 10, scope='net_1')
            self.logits_2, end_points = squeezenet(basic_conv, 10, scope='net_2')
            self.logits_3, end_points = squeezenet(basic_conv, 10, scope='net_3')
            self.logits_4, end_points = squeezenet(basic_conv, 10, scope='net_4')
            self.logits_5, end_points = squeezenet(basic_conv, 10, scope='net_5')
    ...
    def test(self, dataloader, backup_path, epoch, batch_size=128):
        ...
        for i in range(0, dataloader.n_test, batch_size):
            ...
            [avg_accuracy_1,avg_accuracy_2,avg_accuracy_3,avg_accuracy_4,avg_accuracy_5,
            logits_1,logits_2,logits_3,logits_4,logits_5] = self.sess.run(
                fetches=[self.accuracy_1,self.accuracy_2,self.accuracy_3,self.accuracy_4,self.accuracy_5,
                         self.logits_1,self.logits_2,self.logits_3,self.logits_4,self.logits_5],
                feed_dict={self.images:batch_images,
                           self.labels:batch_labels,
                           self.keep_prob:1.0})
            ...
            #几列预测值拼接成矩阵
            merrge_array = np.concatenate([[predict_1],[predict_2],[predict_3],[predict_4],[predict_5]],axis=0)
        ...
        print_and_save_txt(str='test precision_net3: %.4f\n' % (np.mean(accuracy_3_list)),
                           filename=os.path.join(backup_path, 'test_log.txt'))
        print_and_save_txt(str='test precision_net4: %.4f\n' % (np.mean(accuracy_4_list)),
                           filename=os.path.join(backup_path, 'test_log.txt'))
        print_and_save_txt(str='test precision_net5: %.4f\n' % (np.mean(accuracy_5_list)),
                           filename=os.path.join(backup_path, 'test_log.txt'))
