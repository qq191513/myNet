...
class ConvNet():
    ...
    def residual_googlev2_inference(self, images,scope_name):
        with tf.variable_scope(scope_name):
            ...
            with tf.variable_scope('plain_L3.py'):
                with slim.arg_scope([slim.conv2d], stride=1,padding='SAME'):
                        hidden_conv = slim.conv2d(hidden_conv, depth(200), [3, 3], scope='Conv2d_1')
                        # hidden_conv = slim.conv2d(hidden_conv, depth(200), [3, 3], scope='Conv2d_2')
                        # hidden_conv = slim.conv2d(hidden_conv, depth(200), [3, 3], scope='Conv2d_3')
