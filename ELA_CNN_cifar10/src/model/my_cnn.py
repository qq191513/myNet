...
def net_1(hidden_pool1,keep_prob,image_size,n_classes):
    with tf.variable_scope('net_1'):
        ...
        logits_1 = dense_layer2_1.get_output(input=output_dense1_1)
        return logits_1

def net_2(hidden_pool1,keep_prob,image_size,n_classes):
    with tf.variable_scope('net_2'):
        conv_layer2_2 = ConvLayer(
            input_shape=(None, int(image_size / 2), int(image_size / 2), 64), n_size=3, n_filter=128,
            stride=1, activation='relu', batch_normal=True, weight_decay=1e-4,
            name='conv2')
        pool_layer2_2 = PoolLayer(
            n_size=2, stride=2, mode='max', resp_normal=True, name='pool2')

        conv_layer3_2 = ConvLayer(
            input_shape=(None, int(image_size / 4), int(image_size / 4), 128), n_size=3, n_filter=256,
            stride=1, activation='relu', batch_normal=True, weight_decay=1e-4,
            name='conv3')
        pool_layer3_2 = PoolLayer(
            n_size=2, stride=2, mode='max', resp_normal=True, name='pool3')

        dense_layer1_2 = DenseLayer(
            input_shape=(None, int(image_size / 8) * int(image_size / 8) * 256), hidden_dim=1024,
            activation='relu', dropout=True, keep_prob=keep_prob,
            batch_normal=True, weight_decay=1e-4, name='dense1')

        dense_layer2_2 = DenseLayer(
            input_shape=(None, 1024), hidden_dim=n_classes,
            activation='none', dropout=False, keep_prob=None,
            batch_normal=False, weight_decay=1e-4, name='dense2')
        ...
        input_dense1_2 = tf.reshape(hidden_pool3_2, [-1, int(image_size / 8) * int(image_size / 8) * 256])
        output_dense1_2 = dense_layer1_2.get_output(input=input_dense1_2)
        logits_2 = dense_layer2_2.get_output(input=output_dense1_2)
        return logits_2

def net_3(hidden_pool1,keep_prob,image_size,n_classes):
    with tf.variable_scope('net_3'):
        conv_layer2_3 = ConvLayer(
            input_shape=(None, int(image_size / 2), int(image_size / 2), 64), n_size=3, n_filter=128,
            stride=1, activation='relu', batch_normal=True, weight_decay=1e-4,
            name='conv2')
        pool_layer2_3 = PoolLayer(
            n_size=2, stride=2, mode='max', resp_normal=True, name='pool2')

        conv_layer3_3 = ConvLayer(
            input_shape=(None, int(image_size / 4), int(image_size / 4), 128), n_size=3, n_filter=256,
            stride=1, activation='relu', batch_normal=True, weight_decay=1e-4,
            name='conv3')
        pool_layer3_3 = PoolLayer(
            n_size=2, stride=2, mode='max', resp_normal=True, name='pool3')

        dense_layer1_3 = DenseLayer(
            input_shape=(None, int(image_size / 8) * int(image_size / 8) * 256), hidden_dim=1024,
            activation='relu', dropout=True, keep_prob=keep_prob,
            batch_normal=True, weight_decay=1e-4, name='dense1')

        dense_layer2_3 = DenseLayer(
            input_shape=(None, 1024), hidden_dim=n_classes,
            activation='none', dropout=False, keep_prob=None,
            batch_normal=False, weight_decay=1e-4, name='dense2')

        # net 1
        hidden_conv2_3 = conv_layer2_3.get_output(input=hidden_pool1)
        hidden_pool2_3 = pool_layer2_3.get_output(input=hidden_conv2_3)
        hidden_conv3_3 = conv_layer3_3.get_output(input=hidden_pool2_3)
        hidden_pool3_3 = pool_layer3_3.get_output(input=hidden_conv3_3)
        input_dense1_3 = tf.reshape(hidden_pool3_3, [-1, int(image_size / 8) * int(image_size / 8) * 256])
        output_dense1_3 = dense_layer1_3.get_output(input=input_dense1_3)
        logits_3 = dense_layer2_3.get_output(input=output_dense1_3)
        return logits_3
...
class ConvNet():
    ...
    def __init__(self, n_channel=3, n_classes=10, image_size=24):
        ...
        #net_1
        self.logits_1 = net_1(hidden_pool1,self.keep_prob,image_size,n_classes)

        #net 2
        self.logits_2 = net_2(hidden_pool1,self.keep_prob,image_size,n_classes)

        #net 3
        self.logits_3 = net_3(hidden_pool1, self.keep_prob, image_size, n_classes)
