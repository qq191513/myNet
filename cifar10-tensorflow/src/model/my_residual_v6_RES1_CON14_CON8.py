...
class ConvNet():
    def __init__(self, n_channel=3, n_classes=10, image_size=24, n_layers=20):
        ...
        self.logits_2 = self.plain_cnn_inference(images=basic_conv,n_layers = 8, scope_name='net_2')
        self.logits_3 = self.resnet_inference(images=basic_conv,n_layers = 44,scope_name='net_3')
    ...
    def resnet_inference(self,images=None,n_layers = 44,scope_name='net_3'):
        with tf.variable_scope(scope_name):
            n_layers = int((n_layers - 2) / 6)#不是改这里
            # 网络结构
            conv_layer0_list = []
