    ...
    with tf.variable_scope(scope) as sc:
        ...
        with slim.arg_scope([slim.conv2d,slim.avg_pool2d,slim.max_pool2d,_fire_module],
            ...
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
                # conv1
                net = slim.conv2d(inputs, 96, [3,3], stride=2, padding="SAME", scope="conv1")
                ...
                net = _fire_module(net, int(16*compression), scope="fire4")
                
                #
                # net = _fire_module(net, int(16*compression), scope="fire5", use_bypass=use_bypass)
                #
                #
                # net = _fire_module(net, int(32*compression), scope="fire6")
                #
                #
                # net = _fire_module(net, int(32*compression), scope="fire7", use_bypass=use_bypass)
                #
                #
                # net = _fire_module(net, int(32*compression), scope="fire8")
