...
from src.tflayers.conv_layer import Conv_Layer_new_kernel
...
def Conv_new_Net(sess,network_path, n_channel=3, n_classes=10, image_size=24,
                 name_scope=None,weight_dict=None):
    with tf.variable_scope(name_scope):
        # 输入变量
        net_dict = {}
        images_placeholder = tf.placeholder(
        ...
        for layer_dict in network_option['net']['conv_first']:
            ...
            batchsize, w, h, c = weight_dict[k].get_shape()
            c = int(c)
            layer = Conv_Layer_new_kernel(
                x_size=layer_dict['x_size'], y_size=layer_dict['y_size'],
                x_stride=layer_dict['x_stride'], y_stride=layer_dict['y_stride'],
                # n_filter=layer_dict['n_filter'], activation=layer_dict['activation'],
                n_filter=c, activation=layer_dict['activation'],
                batch_normal=layer_dict['bn'], weight_decay=1e-4,
                data_format='channels_last', name=layer_dict['name'],
                input_shape=(image_size, image_size, n_channel),weight=weight)
            conv_lists.append(layer)
...
def train_obeject(dataloader=cifar10, setting = None,net_dict=None,sess=None):
...
def merge_weight(first_weight,second_weight):
    weight_dict = OrderedDict()
    k =0
    for get_first,get_second in zip(first_weight,second_weight):
        if k % 2 == 0:
            concat_axis = 3
        else:
            concat_axis = 2
        weight_dict[k]=tf.concat([get_first,get_second],concat_axis)
        k = k+1
    return weight_dict
