...
class Conv_Layer_new_kernel:

    def __init__(self, y_size, x_size, y_stride, x_stride, n_filter, activation='relu',
                 data_format='channels_last', batch_normal=False, weight_decay=None, name='conv',
                 input_shape=None, prev_layer=None,weight= None):
        # params
        self.y_size = y_size
        self.x_size = x_size
        self.y_stride = y_stride
        ...
        with tf.name_scope('%s_def' % (self.name)) as scope:
            ...
            self.conv = tf.layers.Conv2D(
                ...
                activation=None,
                use_bias=not self.batch_normal,
                kernel_initializer=weight_init_value,
                # kernel_initializer=tf.truncated_normal(shape=[self.y_size,self.x_size], mean=0, stddev=1),
