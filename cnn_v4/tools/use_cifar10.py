...
#     train_data, test_data = color_preprocessing(train_data, test_data)  # 计算有多少批次
#     return train_data, train_labels, test_data, test_labels

def prepare_train_data(is_train=None):
    print("======Loading data======")
    download_data()
    ...
    train_labels = train_labels[indices]
    print("======Prepare Finished======")
    train_data = color_x_train_preprocessing(train_data)  # 计算有多少批次
    return train_data, train_labels


def prepare_test_data(dataset_root):
    print("======Loading data======")
    download_data()
    data_dir = dataset_root
    meta = unpickle(os.path.join(data_dir ,'batches.meta'))
    ...
    print("======Prepare Finished======")
    test_data = color_x_test_preprocessing( test_data)  # 计算有多少批次
...
def color_x_train_preprocessing(x_train):
    x_train = x_train.astype('float32')
    x_train[:, :, :, 0] = (x_train[:, :, :, 0] - np.mean(x_train[:, :, :, 0])) / np.std(x_train[:, :, :, 0])
    x_train[:, :, :, 1] = (x_train[:, :, :, 1] - np.mean(x_train[:, :, :, 1])) / np.std(x_train[:, :, :, 1])
    x_train[:, :, :, 2] = (x_train[:, :, :, 2] - np.mean(x_train[:, :, :, 2])) / np.std(x_train[:, :, :, 2])
...
def color_x_test_preprocessing(x_test):
